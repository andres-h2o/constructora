...
@section('contenido')
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-9">
                <div class="card">
                    <div class="card-header"><h2><strong>Ver Bloqueo</strong></h2> <h1>Puesto <strong>{{ $puesto->numero }}</strong></h1><h4><strong>{{ $puesto->proyecto }}</strong> --- Módulo  : {{ $puesto->modulo }} ---  Bloque  : {{ $puesto->bloque }}</h4></div>
                    <div class="card-body">


                        <form method="POST" action="{{ url('/bloqueo/actualizar/'.$bloqueo->id) }}"
                              accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">
                            {{ csrf_field() }}

                            <div class="form-group {{ $errors->has('dias') ? 'has-error' : ''}}">
                                <div class="col-md-6">
                                    <h3><strong>Fecha : </strong> {{$bloqueo->created_at}}<br><br>
                            ...
                            <div class="form-group {{ $errors->has('estado') ? 'has-error' : ''}}"><h2><strong>
                                ...
                                <div class="col-md-6">
                                    {!! Form::select('estado', [1=>'ACTIVA', 0=>'ANULADA'], $bloqueo->estado, ['class' => 'form-control']) !!}
