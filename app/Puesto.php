...
class Puesto extends Model
{
    ...
    public function scope_getPuestosBloqueados($query, $id_vendedor)
    {
        $puestos = $query->join('bloqueos as blo','blo.id_puesto','puestos.id')
            ->join('categorias as c', 'c.id', 'id_categoria')
            ->join('bloques as b', 'b.id', 'id_bloque')
            ->join('modulos as m', 'm.id', 'b.id_modulo')
            ->join('proyectos as p', 'p.id', 'm.id_proyecto')
            ->where('blo.id_vendedor', '=', $id_vendedor)
            ->select(
                'puestos.id',
                'puestos.nro as numero',
                'largo',
